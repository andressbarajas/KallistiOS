...
static bool run_iteration(uintptr_t tid) {
    ...
    for(unsigned b = 0; b < BARRIER_COUNT; ++b) {
        ...
        printf("Thread[%" PRIuPTR "]: Before barrier[%u]!\n", tid, b);
        ...
        /* Check for errors. */
        if(ret < 0) {
            fprintf(stderr, "Thread[%" PRIuPTR "]: Wait on barrier[%u] failure: %d!\n",
                    tid, b, ret);
            success = false;
        }
        /* Check if we were the lucky serial thread (should only be one!) */
        else if(ret == THD_BARRIER_SERIAL_THREAD) {
            printf("Thread[%" PRIuPTR "]: After barrier[%u]: SERIAL!\n", tid, b);
            ++data[b].serial_barrier_counter;
        }
        else {
            printf("Thread[%" PRIuPTR "]: After barrier[%u]: NONSERIAL!\n", tid, b);
        }
